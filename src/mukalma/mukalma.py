...
class MUKALMA:
    ...
    def __init__(self, params, progress_update_queue):
        ...
        self.parent_topics = None

        # Load list of excluded words from generated dialog model responses
        self.__load_dialog_exclusion_re(params["dialog_exclusion_list"])
    ...
    def __load_dialog_exclusion_re(self, list_fn):
        exclusion_list = []
        with open(list_fn, 'r') as file:
            for line in file:
                exclusion_list.append(str(line).strip())
        self.exclusion_re = rf"\b(?:{'|'.join(exclusion_list)})\b"
        print(f"\n\n[{self.TAG}]: Dialog exclusion re loaded: {self.exclusion_re}\n\n")
    ...
    def generate_knowledge_based_response(self, message, knowledge_sent):
        ...
        if knowledge_sent == "":
            try:
                ...
                print(f"\n\n[{self.TAG}]: EXCEPTION IN generate_knowledge_based_response\nGenerate dialog without"
                      f"knowledge span\n\n")
                return "", []
        ...
        fragmented_outputs = []
        outputs = []
        if len(cloze_responses) > 0:
            cloze_response = cloze_responses[0]

            r_stripped_cloze_response = cloze_response[len(message) + 1:].rstrip()
            if r_stripped_cloze_response[-1] in sent_end_chars:
                r_stripped_cloze_response = r_stripped_cloze_response.rstrip(f' {r_stripped_cloze_response[-1]}')
            ...
            fragmented_output = [(knowledge_sent, dialog)
                                 ...
                                 if re.search(self.exclusion_re, dialog) is None]
        ...
        if len(outputs) == 0:
            outputs.append(cloze_responses[0])
        best_response_id = 0
        best_response = outputs[best_response_id]
        follow_ups = self.dialogue_model.predict(message, output_fragment=best_response)
        follow_up = follow_ups[self.get_best_response_id(follow_ups)]
        print(f"\n\n[{self.TAG}]: \nCOMPLETED RESPONSE: {best_response}\nFOLLOW UP RESPONSE: {follow_up}\n\n")
        if contains_word(follow_up):
            best_response += (' ' if follow_up != '' and follow_up[0] != ' ' else '') + follow_up
    ...
    def get_response(self, message):
        ...
        if not contains_word(best_response):
        ...
        elif self.parent_topics is None:
            self.topic_transition_model.update_topic(best_response, self.__extract_topic(best_response))
    ...
    def set_topic(self, topic):
        print(f"\n[{self.TAG}]: PARENT TOPIC SET TO '{topic}'")
        self.parent_topics = topic
